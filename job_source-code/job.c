...
void scheduler()
{
	...
	if(current == NULL){
		...
		if(current->job->state == DONE |
	   	(current && ((current->job->curpri == 3 && time_temp == 1)|
	   		(current->job->curpri == 2 && time_temp == 2) | 
	  	        (current->job->curpri == 1 && time_temp == 5))
	   	) |
	   	(current && ((current->job->curpri == 3 && head3) | 
	   		(current->job->curpri == 2 && (head2 || head3)) | 
			(current->job->curpri == 1 && (head1 || head2 || head3)))
	   	)
		){
		/* 选择高优先级作业 */
		time_temp = 0;
...
void updateall()
{
	struct waitqueue *p1,*p2,*p3,*prev1,*prev2,*prev3;
	...
	for(p2 = head2,prev2 = head2; p2 != NULL;prev2 = p2, p2 = p2->next){
	...
	for(p1 = head1,prev1 = head1; p1 != NULL;prev1 = p1, p1 = p1->next){
...
int main()
{
	...
	if(stat("/tmp/server",&statbuf_0)==0){
		/* 如果FIFO文件存在,删掉 */
		if(remove("/tmp/server")<0)
			error_sys("remove failed");
	}
